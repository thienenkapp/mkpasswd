...
def ReadWordFile(sCollection, countryFilter=['en'], categoryFilter=['all']):
    ...
    print(f"Started ReadWordFile: {generated_uuid}")
    pprint.pprint(countryFilter)
    pprint.pprint(categoryFilter)
    print("wordnoun       "+ str(len(wordsnoun)))
    print("wordverbs      "+ str(len(wordsverb)))
    print("wordsadjective "+ str(len(wordsadjective)))
    ...
    for doc in docs:
        ...
        if (useData==True) and ((categoryFilter=="all") or (category_name in categoryFilter)):
            useData=True
            #print("Category: Ok " + category_name)
        else:
            useData= False
        ...
        if ("nouns" in data):
            ...
            print("No NOUN defined - Check " + category_name)

        if ("verbs" in data):
            wordsverb.extend(data['verbs'])
            #print(wordsverb)
        else:
            print("No VERB defined - Check " + category_name)

        if ("adjectives" in data):
            wordsadjective.extend(data['adjectives'])
            #print(wordsadjective)
        else:
            print("No ADJECTIVE defined - Check " + category_name)

    print("wordnoun       "+ str(len(wordsnoun)))
    print("wordverbs      "+ str(len(wordsverb)))
    print("wordsadjective "+ str(len(wordsadjective)))
    print(f"Done ReadWordFile: {generated_uuid}")
...
def main_get_passwords(request):
    ...
    country = request.args.get('country', default = 'en', type = str)
    category = request.args.get('category', default = 'eff', type = str)
